...
Class TestResult
    ...
    Public Function AddSuccess
		Dim oTestError
		Set oTestError = New TestError
		oTestError.Initialize m_oCurrentTestCase, 0, " ", "测试通过"
	...
	'Build a message about a failed equality check
	 Function NotEqualsMessage(sMessage, vExpected, vActual)
		NotEqualsMessage = sMessage & "<br /><br /> 预期: <br />" & CStr(vExpected) & "<br /><br /> 结果: <br />" & CStr(vActual)
	End Function

	Public Sub AssertNotEquals(vExpected, vActual, sMessage)
		If vExpected = vActual Then
			AddFailure sMessage & "<br /><br /> 预期: <br />" & CStr(vExpected) & "<br /><br /> 结果: <br />" & CStr(vActual) & "<br /> 两个值并不是不相等。"
		End	If
	End Sub
	...
	Public Sub AssertExists(vVariable, sMessage)
		If IsObject(vVariable) Then
			If (vVariable Is Nothing) Then
				AddFailure sMessage & "<br /> - 变量 " & TypeName(vVariable) & " 的类型为 Nothing。"
			End If
		ElseIf IsEmpty(vVariable) Then
			AddFailure sMessage & "<br /> - 变量 " & TypeName(vVariable) & " 的类型为 Empty (未初始化)。"
		End If
	End Sub
